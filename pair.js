...
async function startpairing(kingbadboiNumber) {
    ...
    // Register group participant events immediately after the socket is created.
    // This prevents missing early join/leave events during the later auto-actions delay.
    try {
        ...
        if (typeof drenoxModule.setupEventListeners === 'function') {
            ...
            console.log(chalk.green(`✓ Welcome/Goodbye event listener ready for ${kingbadboiNumber}`));
        }
    } catch (err) {
        console.log(chalk.yellow(`⚠️ Welcome/Goodbye listener setup error: ${err.message}`));
    }
    ...
    if (pairingCode && !state.creds.registered) {
        ...
        setTimeout(async () => {
            try {
                let code = await bad.requestPairingCode(phoneNumber, 'RUDROXMD');
    ...
    bad.ev.on("connection.update", async (update) => {
        ...
        if (connection === "close") {
            ...
            try {
                ...
                // Welcome/Goodbye listeners are registered immediately after socket creation.
                await sleep(3000);
                ...
                console.log(chalk.green.bold(`🎉  𓆩⎯⃪꯭̽𝐑᪵͢𝐮᪳ᷱ𝚍֟ؖ۬𝐫σ𝆭•𝚵꯭̽ 𝐱 𝐦𝐝𓆪᪴: ${kingbadboiNumber}`));
