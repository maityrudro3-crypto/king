...
const checkUserJoinedChannels = async (userId) => {
  const channels = ['@rudro_allhack', '@Rudeo_x_bot'];
  let allJoined = true;
...
const sendChannelsRequiredMessage = async (chatId) => {
  return bot.sendMessage(chatId,
    `🚨 *You must join our official channels before pairing.*`,
    {
      ...
      reply_markup: {
        inline_keyboard: [
          [{ text: '📢 Channel 1', url: 'https://t.me/Rudeo_x_bot' }],
          [{ text: '📢 Channel 2', url: 'https://t.me/rudro_allhack' }],
          [{ text: '👥 Group', url: 'https://t.me/rudro_xmd' }],
...
bot.onText(/\/start/, async (msg) => {
  ...
  await bot.sendPhoto(
    ...
    {
      caption: `🪀 *𝙏𝙝𝙚 𓆩⎯⃪꯭̽𝐑᪵͢𝐮᪳ᷱ𝚍֟ؖ۬𝐫σ𝆭•𝚵꯭̽ 𝐱 𝐦𝐝𓆪᪴💀*\n\n╔════════════════════╗\n ⤷ /pair <wa_number>\n ⤷ /unpair <wa_number>\n╚════════════════════╝`,
      parse_mode: 'Markdown',
      reply_markup: {
        inline_keyboard: [
          [{ text: "👑 Owner", url: "https://t.me/King_rudro" }]
        ]
      }
    }
  );
...
(async () => {
  ...
  console.log('✅ Bot Username: @free_allhack_bot');
